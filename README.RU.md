...
построенный на Tailwind CSS v4 и shadcn/ui v4 (27 компонентов), с формой входа,
...
- **Управление пользователями** — TanStack Table с поиском, фильтрацией по статусу/email/ролям, пагинацией с выбором размера страницы, диалогами создания/редактирования/управления ролями/смены пароля/подтверждения удаления, восстановлением soft-deleted пользователей
...
npm run test:coverage # С отчётом о покрытии (7.35% всего)
npm run test:e2e      # Playwright E2E тесты — 8 файлов, ~979 строк
```
...
| Покрытие тестами (бэкенд) | **13.3%** в целом — `domain/user` 86.6%, `rbac` 97.3%, `ratelimit` 76.6%, `config` 41.4% |
| LOC бэкенда | 13 376 строк Go (78 файлов) |
...
| UI-компоненты | shadcn/ui v4 (base-nova, 27 компонентов) |
...
| Покрытие тестами (фронтенд) | **7.35%** (операторы) — `lib/` 43.87%, страницы/UI 0% |
| LOC фронтенда | 6 648 строк TS/TSX (55 файлов) |
