...
пользователями, файловыми аватарами пользователей, управлением настройками пользователей,
rate-limited эндпоинтом входа, планировщиком cron-задач и очисткой
просроченных токенов. Фронтенд — TypeScript admin dashboard на React 19,
построенный на Tailwind CSS v4 и shadcn/ui v4 (29 компонентов), с формой входа,
сайдбар-навигацией, CRUD-таблицей пользователей, страницей настроек
(combobox/switch/slider/radio-inputs), загрузкой аватаров пользователей
и переключением тёмной/светлой темы.
...
- **Управление настройками пользователей** — определения настроек (группы, типы), CRUD настроек пользователей, 47 предустановленных настроек в 5 группах
- **Аватары пользователей** — загрузка аватаров через multipart (макс. 2MB, 200x200), выдача и удаление
...
- **Admin dashboard** — сайдбар-навигация со сворачиваемым меню, адаптивный макет, аватар пользователя в сайдбаре/хэдере
- **Страница входа** — форма с валидацией (zod + react-hook-form), сохранение токена
- **Управление пользователями** — TanStack Table с поиском, фильтрацией по статусу/email/ролям, пагинацией с выбором размера страницы, диалогами создания/редактирования/управления ролями/смены пароля/подтверждения удаления, восстановлением soft-deleted пользователей, загрузкой аватаров
- **Страница настроек** — настройки, сгруппированные по группам и секциям, combobox/switch/slider/radio-inputs, сохранение
...
- **Go-бэкенд** предоставляет RESTful JSON API через Gin на `/api/v1/`, подключается
  ...
  файловые аватары пользователей, управление настройками пользователей,
...
- **Next.js-фронтенд** потребляет API и рендерит admin dashboard на React 19.
  ...
  темы/меню пользователя (с аватаром), страницу входа с валидацией zod, CRUD-таблицу
  пользователей на TanStack Table с поиском/фильтрами по статусу, email и ролям, пагинацией с выбором размера страницы,
  диалогами создания/редактирования/управления ролями/смены пароля/подтверждения удаления, восстановлением soft-deleted пользователей,
  страницу настроек с combobox/switch/slider/radio-полями ввода,
...
cd front
source /home/bookworker06JAN1979/.nvm/nvm.sh && nvm use
npm test              # Vitest unit tests (с MSW) — 16 тестов
npm run test:coverage # С отчётом о покрытии (5.46% всего)
npm run test:e2e      # Playwright E2E тесты — 12 файлов
```
...
| Покрытие тестами (бэкенд) | **11.9%** в целом — `domain/user` 86.6%, `domain/settings` ~77%, `rbac` 97.3%, `ratelimit` 76.6%, `config` 96.8% |
| LOC бэкенда | 17 512 строк Go (97 файлов) |
...
| UI-компоненты | shadcn/ui v4 (base-nova, 29 компонентов) |
...
| Покрытие тестами (фронтенд) | **5.46%** (операторы) — `lib/` 33.17%, страницы/UI 0% |
| LOC фронтенда | 8 230 строк TS/TSX (62 файла) |
